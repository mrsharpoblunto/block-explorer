...
export class World {
    ...
    mountSimSystems(systems: Array<SimSystem>): void {
        for (let system of this._simSystems) {
            if (system.systemWillUnmount) {
                system.systemWillUnmount(this,this._canvas);
            }
        }
        this._simSystems = systems;
        for (let system of this._simSystems) {
            if (system.systemWillMount) {
                system.systemWillMount(this,this._canvas);
            }
        }
    ...
    render(alpha: number): void {
        this._glContext.clearColor(0.4,0.8,1.0,1.0);
    ...
    createEntity(builder: (entity: Entity, glContext: any, options: any) => Entity, options: any): Entity {
        ...
        this._building = true;
        const entity = builder(new Entity(id,this),this._glContext,options);
        this._building = false;
